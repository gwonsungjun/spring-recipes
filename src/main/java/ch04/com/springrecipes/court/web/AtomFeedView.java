package ch04.com.springrecipes.court.web;
...
public class AtomFeedView extends AbstractAtomFeedView {
    ...
    protected void buildFeedMetadata(Map model, Feed feed, HttpServletRequest request) {
        ...
        List<TournamentContent> tournamentContentList = (List<TournamentContent>) model.get("feedContent");

        feed.setUpdated(tournamentContentList.stream().map(TournamentContent::getPublicationDate).sorted().findFirst().orElse(null));
    }

    @Override
    protected List<Entry> buildFeedEntries(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        List<TournamentContent> tournamentContentList = (List<TournamentContent>) model.get("feedContent");
        return tournamentContentList.stream().map(this::toEntry).collect(Collectors.toList());
    }
